    ...
    >
      <div className="flex flex-col justify-between h-full p-5">
        <div className="flex justify-between items-center mb-5 border-b border-slate-300 pb-5">
          <h2
            className={`text-2xl uppercase font-bold text-gray-600 transition-opacity duration-300 ${
              isOpen ? 'opacity-100' : 'opacity-0'
            }`}
          >
            <span className="text-white">Kaah</span>{' '}
            <span className="text-red-300">Party</span>
          </h2>
          <Button
            ...
            <NavLink
              className={({ isActive }) =>
                isActive && isOpen
                  ...
                  : isActive && !isOpen
                  ? ' text-white  py-2'
                  : 'flex items-center gap-2 font-semibold text-slate-400'
              }
              to="/"
            >
              <LayoutDashboard />
